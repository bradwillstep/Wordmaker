...
(function(){
  ...
  // ---------- Service worker (strong update + auto-reload) ----------
  (async function registerSW(){
    if(!("serviceWorker" in navigator)) return;

    // Auto-reload once a new SW takes control (most reliable way to ensure everyone gets the newest build)
    let reloading = false;
    ...
    try{
      ...
      reg.addEventListener("updatefound", () => {
        ...
        sw.addEventListener("statechange", () => {
          // When installed, if we already have a controller, it's an update. Activate immediately.
